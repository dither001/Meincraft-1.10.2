...
public class ModelLizardmanStalker extends ModelBase {
...
//	    ModelRenderer leftBracer;
	  ...
	  {
	  //f3 = netheadyaw
	  EntityLizardmanStalker nttLiz = (EntityLizardmanStalker)ntt;
	  ...
	  //walking
		...
		/*	  } else {
				  this.TailSeg1.rotateAngleY = (float) (MathHelper.sin((float) (age / 2/Math.PI)+nttLiz.getEntityId()) * 0.3);
				  this.TailSeg2.rotateAngleY = (float) (MathHelper.sin((float) (-1F + (age / 2/Math.PI)+nttLiz.getEntityId())) * 0.3);
				  this.TailSeg3.rotateAngleY = (float) (MathHelper.sin((float) (-1.6F + (age / 2/Math.PI)+nttLiz.getEntityId())) * 0.3);
				  } */
		  ...
		  //chatting
		 int k = rng.nextInt(120)+120;
//		 if (t == 0)
		  {
			this.LowerJaw.rotateAngleX = -MathHelper.sin((float) ((t/Math.PI) * 0.25F + 44/rad)) * 2.5f;// - motion/2;
			} //else {} //rng.nextInt(30)+30;}
		  ...
		  //combat related.
		  //add left arm pose for aimed bow + default

		  //System.out.println(lt);

		  //has a target, then if timer is active, will shoot and draw bow
		  if(aiming == 2) {
    		  this.upperRarm.rotateAngleX = -85/rad + (this.head.rotateAngleX + 30/rad);
	    	  this.upperRarm.rotateAngleY = (-0.17325F - this.body.rotateAngleY);// + this.head.rotateAngleY;
			  this.lowerRarm.rotateAngleX = -30/rad;
			  
			  this.lowerbody.rotateAngleY = -13/rad;
			  this.head.rotateAngleY = 50/rad;
			  this.body.rotateAngleY = -10/rad;
			  
			  this.upperLarm.rotateAngleX = -5/rad + (this.head.rotateAngleX + 30/rad);// - (this.head.rotateAngleY*.25f);
			  this.upperLarm.rotateAngleY = 0.0F;// - (this.head.rotateAngleY);// + this.head.rotateAngleY;
    		  this.upperLarm.rotateAngleZ = -0.7091F;
			  this.lowerLarm.rotateAngleX = -124/rad - (this.head.rotateAngleX + 30/rad);// + (this.head.rotateAngleY*.25f);
			  this.lowerLarm.rotateAngleY = 0.0F;// + (this.head.rotateAngleY);
			  this.lowerLarm.rotateAngleZ = -5/rad;
			  this.Lefthand.rotateAngleZ = 37/rad;

			  if(swing > 0)
			  {
	    		  this.upperLarm.rotateAngleX = -5/rad - ((float) MathHelper.sin(swing*0.1F)) * 1.2F
	    				 + (this.head.rotateAngleX + 30/rad);
				  this.upperLarm.rotateAngleY = 0.F - ((float) MathHelper.sin(swing*0.1F)) * .2F;
	    		  this.upperLarm.rotateAngleZ = -0.7091F + ((float) MathHelper.sin(swing*0.1F)) * .52F;
				  this.lowerLarm.rotateAngleX = -124/rad + ((float) MathHelper.sin(swing*0.1F)) * 1.2F
						  - (this.head.rotateAngleX + 30/rad);
				  this.lowerLarm.rotateAngleY = 0F + ((float) MathHelper.sin(swing*0.1F)) * .25F;
				  this.lowerLarm.rotateAngleZ = (-5/rad + ((float) MathHelper.sin(swing*0.1F)) * .52F);
				  this.Lefthand.rotateAngleZ = 37/rad;
			  }	
		  }
			  
			  else
				  if(aiming == 5) //no target
			  {
	    		  ...
	    		  this.upperRarm.rotateAngleX = motion/1.5F - 40/rad; //((barney*5) - 40/rad)/(6);//this.weighted(this.upperRarm.rotateAngleX, -40/rad, 5);
	    		  this.upperRarm.rotateAngleY = 0.325F;
	    		  
	    		  this.upperRarm.rotateAngleZ = 0.1927091F; //-0.7091F;

	    		  this.lowerRarm.rotateAngleX = -30/rad;
	    		  
	    		  
				  this.upperLarm.rotateAngleX = motion/1.5F - -5/rad;
				  this.upperLarm.rotateAngleY = -0.325F;
	    		  this.upperLarm.rotateAngleZ = -0.1927091F;

				  this.lowerLarm.rotateAngleX = -motion - 30/rad;
					  this.lowerLarm.rotateAngleY = 0.0297429F;
					  this.lowerLarm.rotateAngleZ = -6/rad;	    	  
				  this.body.rotateAngleY = -0/rad;
	    		  
	    		  } else {
	    			  this.upperRarm.rotateAngleX = motion/1.5F - -2/rad;
			    	  this.upperRarm.rotateAngleY = 0.0F;
					  ...
					  this.upperLarm.rotateAngleX = motion/1.5F - -5/rad;
					  this.upperLarm.rotateAngleY = 0.0F;
		    		  this.upperLarm.rotateAngleZ = -0.7091F;
	  ...
	  public void setLivingAnimations(EntityLivingBase ntt, float time, float motion, float par4) {
		  	        //store the bow on the back. have it pulled when the stalker has a hostile target.
	      //setup stuff for attack animation, and have bow held lower while not aggro'd. 
	  }
	  //try moving this to the entity file.
