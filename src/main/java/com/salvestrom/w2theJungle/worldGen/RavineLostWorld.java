...
public class RavineLostWorld extends MapGenBase
{
    ...
    {
        if (biome instanceof BiomeMushroomIsland) return true;
        if (biome instanceof BiomeBeach) return true;
        if (biome instanceof BiomeDesert) return true;
        return false;
    }
    ...
    {
        Biome biome = worldObj.getBiome(new BlockPos(x + chunkX * 16, y, z + chunkZ * 16));
    ...
    protected void digBlock(Block[] data, int index, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop)
    {
        ...
        if (block == Blocks.STONE || block == filler || block == top)
        {
            if (y < 10)
            {
                data[index] = Blocks.FLOWING_LAVA; //TODO make mud or water, or stone??
            }
            else
            {
                data[index] = null;
